  ...
  describe('rules', () => {
    describe('when password has one letter', () => {
      ...
      it('should highlight one letter is satisfied', () => {
        expect(wrapper.get('[data-test-rule="OneLetter"]').classes('is-valid')).toBe(true);
      });
      it('should not highlight number satisfied', () => {
        expect(wrapper.get('[data-test-rule="OneNumber"]').classes('is-valid')).toBe(false);
      });
    });
    ...
    describe('when password has lower an upper letters', () => {
      ...
      it('should highlight upper and lower letter is satisfied', () => {
        expect(wrapper.get('[data-test-rule="UpperAndLower"]').classes('is-valid')).toBe(true);
      });
      it('should not highlight number satisfied', () => {
        expect(wrapper.get('[data-test-rule="OneNumber"]').classes('is-valid')).toBe(false);
      });
